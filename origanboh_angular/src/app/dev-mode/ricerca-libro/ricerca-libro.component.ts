...
export class RicercaLibroComponent {
  ...
  getLibriByName(){
    ...
    this.http.get<Libro[]>("http://localhost:8080/api/libro/searchByName", {headers, params}).subscribe(risposta =>{
      this.libri = risposta;
  ...
  getAllLibri(){
    ...
    this.http.get<Libro[]>("http://localhost:8080/api/libro/all", {headers}).subscribe(risposta =>{
      this.libri = risposta;
      this.search.emit(this.libri);
    })
  }
